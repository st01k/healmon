...
(function (win, doc, undefined)
{
    ...
    RG.SVG.createSVG = function (opt)
    {
        ...
        var group = RG.SVG.create({
            ...
            attr: {
                ...
                _transform: 'skewY(5)' // This makes the chart appear 3dish
            }
        });
                ...
                if (prop.xaxisLabelsPosition === 'section') {
                    ...
                    for (var i=0; i<prop.xaxisLabels.length; ++i) {
                        ...
                        if (obj.scale.max <=0 && obj.scale.min < obj.scale.max) {
                            var y = prop.gutterTop - (RG.SVG.ISFF ? 5 : 10) + prop.xaxisLabelsOffsety;
                            var valign = 'bottom';
                        } else {
                            var y = obj.height - prop.gutterBottom + (RG.SVG.ISFF ? 5 : 10) + prop.xaxisLabelsOffsety;
                            var valign = 'top';
                        }
                    ...
                    for (var i=0; i<prop.xaxisLabels.length; ++i) {
                        ...
                        if (obj.scale.max <= 0 && obj.scale.min < 0) {
                            valign = 'bottom';
                            y = prop.gutterTop - (RG.SVG.ISFF ? 5 : 10) - (prop.xaxisTickmarksLength - 5) + prop.xaxisLabelsOffsety
                        } else {
                            valign = 'top';
    ...
    RG.SVG.drawBackground = function (obj)
    {
        ...
        if (prop.backgroundImage) {
            ...
            if (prop.backgroundImageStretch) {
                ...
                attr.x      = prop.gutterLeft;
                attr.y      = prop.gutterTop;
                ...
                if (typeof prop.backgroundImageX === 'number') {
                    attr.x =  prop.backgroundImageX;
                }

                if (typeof prop.backgroundImageY === 'number') {
                    attr.y =  prop.backgroundImageY;
                }
    ...
    */
    ...
    RG.SVG.clear = function (svg)
    {
        ...
        //    svg.all.removeChild(svg.lastChild);
        //}
        //
        while (svg.lastChild) {
    ...
    */
    ...
    RG.SVG.attribution = function (obj)
    {
        ...
        var text = RG.SVG.text({
            object: obj,
            parent: a,
            text:   typeof prop.attributionText === 'string' ? prop.attributionText : 'HTML5 charts by RGraph',
            x:      x,
            y:      y,
